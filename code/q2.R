...
data_mod_q2 <- nobel %>%
  mutate(decade = case_when(year<1910 ~ "1900-1909", year>1909 & year<1920 ~ "1910-1919", 
                            ...
                            year>1999 & year<2010 ~ "2000-2009", year>2009 & year<2020 ~ "2010-2019", year>2019 ~ "2020-2023"))
...
#make a plot
q2_plot_I <- ggplot(data=q2_data, aes(x=decade, y=prop)) + geom_col() +
  labs(title="Proportion of US-Born Nobel Laureates per Decade", x='Decade',
       y='Proportion of US-Born Nobel Winners (%)') +
  theme(axis.text.x=element_text(angle=90, size=7), axis.text.y=element_text(size=8),
        plot.title=element_text(size=11), axis.x.title=element_text(size=9),
        axis.y.title=element_text(size=9))
q2_plot_I